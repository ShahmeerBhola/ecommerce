...
class Wf_Woocommerce_Packing_List {
	...
	*/
    ...
    public static function generate_print_button_for_user($order,$order_id,$action,$label,$email_button=false)
    {
		...
		$invoice_url=esc_url(site_url('?attaching_pdf=1&print_packinglist=true&email='.$billing_email_enc.'&post='.$order_id_enc.'&type='.$action.'&user_print=1&_wpnonce='.$_nonce.$locale));
        $style='';
        if($email_button)
        {
