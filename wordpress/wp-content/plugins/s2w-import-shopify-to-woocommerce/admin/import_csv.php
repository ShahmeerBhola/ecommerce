...
class S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_ADMIN_Import_Csv {
	...
	public function admin_notices() {
		if ( $this->process->is_downloading() ) {
			?>
            <div class="updated">
                <p>
			...
			<?php
		} elseif ( ! $this->process->is_queue_empty() ) {
			?>
            <div class="updated">
                <p>
	...
	public function add_menu() {
		add_submenu_page(
			's2w-import-shopify-to-woocommerce', __( 'Import CSV', 's2w-import-shopify-to-woocommerce' ), __( 'Import CSV', 's2w-import-shopify-to-woocommerce' ), 'manage_options', 's2w-import-shopify-to-woocommerce-import-csv', array(
				$this,
				'import_csv_callback'
			)
		);
			...
			if ( ( $file_handle = fopen( $file_url, "r" ) ) !== false ) {
				...
				while ( ( $item = fgetcsv( $file_handle, 0, "," ) ) !== false ) {
					...
					if ( false === $next_item ) {
						/*create previous product*/
						$this->import_product( $new_product, $import_options );
						if ( $import_options['download_images'] && $import_options['download_images_later'] ) {
							$this->process->dispatch();
						}
						fclose( $file_handle );
						wp_send_json( array(
