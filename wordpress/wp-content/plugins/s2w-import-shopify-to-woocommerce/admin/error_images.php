...
if ( ! class_exists( 'S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_ADMIN_Error_Images' ) ) {
	class S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_ADMIN_Error_Images {
		...
		public function download_error_product_images() {
			...
			if ( $id ) {
				$row = S2W_Error_Images_Table::get_row( $id );
				if ( count( $row ) ) {
					$data       = $row[0];
		...
		public function admin_menu() {
			$import_list = add_submenu_page( 's2w-import-shopify-to-woocommerce', esc_html__( 'Failed Images', 's2w-import-shopify-to-woocommerce' ), esc_html__( 'Failed Images', 's2w-import-shopify-to-woocommerce' ), 'manage_options', 's2w-import-shopify-to-woocommerce-error-images', array(
				$this,
				'page_callback'
			) );
                    ...
                    <form class="vi-ui form">
                        <table class="vi-ui celled table">
                            ...
                            <tr>
                                ...
                                <th><?php esc_html_e( 'Set gallery', 's2w-import-shopify-to-woocommerce' ) ?></th>
									...
									foreach ( $result as $result_k => $result_v ) {
                                        ...
                                        <td>
											...
											if ( $result_k === 'image_src' ) {
												?>
                                                <img width="48" height="48" src="<?php echo esc_url( $result_v ) ?>">
												<?php
											} else {
