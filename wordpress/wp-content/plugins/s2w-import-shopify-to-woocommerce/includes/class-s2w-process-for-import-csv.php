...
class WP_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_Process_For_Import_Csv extends WP_Background_Process {
	...
	protected function task( $item ) {
		...
		try {
			...
			if ( $product_id && $src ) {
				...
				if ( $post && $post->post_type === 'product' ) {
					...
					if ( $thumb_id && ! is_wp_error( $thumb_id ) ) {
						...
						S2W_Error_Images_Table::insert( $product_id, implode( ',', $product_ids ), $src, $alt, intval( $set_gallery ) );
						if ( is_wp_error( $thumb_id ) ) {
							error_log( 'S2W error log - background download images for "import csv": ' . $thumb_id->get_error_code() . ' - ' . $thumb_id->get_error_message() );
						}
					}
				}
			}
			...
			S2W_Error_Images_Table::insert( $product_id, implode( ',', $product_ids ), $src, $alt, intval( $set_gallery ) );
			error_log( 'S2W error log - background download images for "import csv": ' . $e->getMessage() );
	...
	protected function complete() {
		if ( ! $this->is_process_running() && $this->is_queue_empty() ) {
			set_transient( 's2w_background_processing_complete_for_import_csv', time() );
		}
	 ...
	 * Delete all batches.
	 *
	 * @return WP_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_Process_For_Import_Csv
	 */
