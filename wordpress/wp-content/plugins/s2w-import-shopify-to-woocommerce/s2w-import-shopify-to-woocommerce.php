...
define( 'VI_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_VERSION', '1.0.9.5' );
...
if ( ! class_exists( 'S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE' ) ) {
	class S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_order_number', array( $this, 'woocommerce_order_number' ) );
		...
		public function woocommerce_order_number( $order_id ) {
			$order_number = get_post_meta( $order_id, '_s2w_shopify_order_number', true );
			if ( $order_number ) {
				return $order_number;
			}
		...
		public function parse_query( $query ) {
			global $pagenow;
			if ( $pagenow == 'edit.php' && isset( $_GET['post_type'] ) && $_GET['post_type'] && isset( $_GET['s2w_import_orders'] ) && $_GET['s2w_import_orders'] ) {
		...
		public function plugins_loaded() {
			$this->process_new = new WP_S2W_IMPORT_SHOPIFY_TO_WOOCOMMERCE_Process_New();
									...
									if ( is_array( $customer ) && count( $customer ) ) {
										...
										if ( $existing_id ) {
											...
											if ( ! is_wp_error( $user_id ) ) {
												...
												$wc_customer->set_billing_phone( $customer_data['phone'] );
												$wc_customer->set_billing_company( $customer_data['company'] );
												$wc_customer->set_shipping_company( $customer_data['company'] );
												$wc_customer->set_billing_address_1( $customer_data['address1'] );
												$wc_customer->set_shipping_address_1( $customer_data['address1'] );
												$wc_customer->set_billing_address_2( $customer_data['address2'] );
												$wc_customer->set_shipping_address_2( $customer_data['address2'] );
												$wc_customer->set_billing_city( $customer_data['city'] );
												$wc_customer->set_shipping_city( $customer_data['city'] );
												$wc_customer->set_billing_state( $customer_data['province'] );
												$wc_customer->set_shipping_state( $customer_data['province'] );
												$wc_customer->set_billing_country( $customer_data['country'] );
												$wc_customer->set_shipping_country( $customer_data['country'] );
												$wc_customer->set_billing_postcode( $customer_data['zip'] );
												$wc_customer->set_shipping_postcode( $customer_data['zip'] );
												$wc_customer->save();
											}
										}
									}
...
//				create order coupon lines
																...
																if ( $description && $download_description_images ) {
																	...
																	if ( isset( $matches[1] ) && is_array( $matches[1] ) && count( $matches[1] ) ) {
																		...
																		foreach ( $description_images as $description_image ) {
																			...
																			$this->process_new->push_to_queue( $images_data );
																		}
																	}
																}
		...
		public function admin_menu() {
			add_menu_page( esc_html__( 'Import Shopify to WooCommerce', 's2w-import-shopify-to-woocommerce' ), esc_html__( 'Shopify to Woo', 's2w-import-shopify-to-woocommerce' ), 'manage_options', 's2w-import-shopify-to-woocommerce', array(
				$this,
				'settings_callback'
		...
		public function admin_menu_system_and_log() {
			...
			add_submenu_page(
				...
				esc_html__( 'System Status', 's2w-import-shopify-to-woocommerce' ),
				'manage_options',
				's2w-import-shopify-to-woocommerce-status',
